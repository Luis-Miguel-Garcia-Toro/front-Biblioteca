        ...
        {libros.length > 0 ? (
          <>
            {libros?.map((book) => (
              ...
              >
                <Link
                  to={`/libro/${book.id_libro}`}
                  state={{ from: window.location.pathname }}
                >
                  <img
                    src={book.imagenLibro}
                    alt={book.nombre}
                    className="h-48 w-auto object-cover rounded-t-lg mb-4"
                  />
                </Link>
                <div>
                  <p className="text-md sm:text-lg font-semibold text-white mb-2">{book.nombre}</p>
                  <p className="text-gray-300 text-sm">{book.autor && book.autor}</p>
                </div>
              </div>
            ))}
          </>
